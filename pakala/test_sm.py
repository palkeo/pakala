...
class TestSymbolicMachine(unittest.TestCase):
    ...
    def test_add_for_fuzzing(self):
        ...
        self.sm.add_for_fuzzing(state, variable, [min, max, None, None, None, None, 42, 1337])
...
class TestInstructions(unittest.TestCase):
    ...
    def test_eq(self):
        ...
        self.assert_stack([False])
        self.run_code(['PUSH1', claripy.BoolV(False), 'PUSH1', claripy.BoolV(False), 'EQ'])
        self.assert_stack([True])
        self.run_code(['PUSH1', claripy.BoolV(True), 'PUSH1', claripy.BoolV(False), 'EQ'])
        self.assert_stack([False])
    ...
    def test_blockhash(self):
        ...
        self.assert_stack([
            self.state.env.block_hashes[self.state.env.block_number]])
    ...
    def test_jumpi(self):
        r = self.run_code(['PUSH1', 1337, 'CALLVALUE', 'EQ',
                           'PUSH1', 8,
                           'JUMPI',
                           'PC',
                           'JUMPDEST'])
    ...
    def test_calldatacopy(self):
        # size, dstart, mstart
        self.run_code(['PUSH1', 64, 'PUSH1', 20, 'PUSH1', 10, 'CALLDATACOPY',
                       'PUSH1', 10, 'MLOAD', 'PUSH1', 42, 'MLOAD'])
        self.assertEqual(self.state.solver.min(self.state.env.calldata_size), 84)
        self.assert_stack([self.state.env.calldata.read(20, 32),
                           self.state.env.calldata.read(52, 32)])
    ...
    def test_codecopy(self, mock_disassemble):
        code = ['CODESIZE', 'PUSH1', 0, 'DUP1', 'CODECOPY',
                'PUSH1', 0, 'MLOAD']
        mock_disassemble.return_value = code
        self.sm = sm.SymbolicMachine(Env(
            b'\x60' + bytearray(len(code) - 2) + b'\xa1'))
        self.state = self.sm.branch_queue[0][1]
        self.sm.exec_branch(self.state)
    ...
    def test_mstore(self):
        ...
        self.assert_stack([1337])
        self.run_code(['PUSH1', 0xcafe, 'PUSH1', 64, 'MSTORE',
                       'PUSH1', 0xdead, 'PUSH1', 62, 'MSTORE',
                       'PUSH1', 66, 'MLOAD'])
